...
let btnOpenCookie = document.querySelector("button");
let phrases = `A vida trará coisas boas se tiveres paciência.  
...
closedCookie.addEventListener("click", randomPhrase);
btnOpenCookie.addEventListener("click", toggleScreen);
btnOpenCookie.addEventListener("keydown", enter);

// Funções
function randomPhrase() {
  indexPhrase = Math.round(Math.random(1) * phrases.length);
  phrase = phrases[indexPhrase];
  console.log(indexPhrase);
  console.log(phrase);
  paper.innerHTML = phrase;
